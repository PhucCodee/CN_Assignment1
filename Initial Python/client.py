...
class Client:
    ...
    def send_request(self, request, expect_response=True):
        try:
            ...
            print(f"Socket error during send_request: {e}")
        except Exception as e:
    ...
    def download_file(self, file_name, save_location):
        ...
        try:
            ...
            response = self.send_request(request)
            print(f"Response from tracker: {response}")  # Debug

            if response:
                response = json.loads(response)
                print(f"Parsed response: {response}")  # Debug

                if response["status"] == "success":
                    file_hash = response.get("file_hash")
                    magnet_link = response.get("magnet_link")
                    print(
                        f"File hash: {file_hash}, Magnet link: {magnet_link}"
                    )  # Debug

                    missing_pieces = response.get("missing_pieces", [])
                    print(f"Missing pieces: {missing_pieces}")  # Debug
            ...
            print(f"Socket error in download_file: {e}")
        except Exception as e:
            print(f"Error during download_file: {e}")
        finally:
            print("Download file process completed.")
